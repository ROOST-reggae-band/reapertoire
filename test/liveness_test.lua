...
-- A track whose sound is generated by a plugin from MIDI has no readable
-- audio at the item level, so its frames are entirely absent. Judged on
-- frames alone it reads as a dead track and disappears from every take.

function T.a_programmed_track_with_items_is_live()
  local frames = {}
  for i = 1, 100 do frames[i] = false end
  local r = liveness.classify(frames, OPTS, { programmed = true, has_items = true })
  h.assert_eq(r.live, true)
  h.assert_eq(r.floor_db, nil, "no floor to compute without levels")
end

function T.a_programmed_track_with_no_items_is_not_live()
  local frames = {}
  for i = 1, 100 do frames[i] = false end
  local r = liveness.classify(frames, OPTS, { programmed = true, has_items = false })
  h.assert_eq(r.live, false)
end

function T.an_ordinary_silent_track_is_still_not_live()
  -- The programmed path must not become a way for any empty track to pass.
  local frames = {}
  for i = 1, 100 do frames[i] = false end
  h.assert_eq(liveness.classify(frames, OPTS).live, false)
end
